...
knn_conf_matrix <- table(knn3pred[,2], task1_test$label)
...
table_pca <- rbind(pca$rotation[,1:10], summary(pca)$importance[,1:10])
...
pca_knn <- knn3Train(reduced_train[2:ncol(reduced_train)], 
                 reduced_test[2:ncol(reduced_test)], 
                 reduced_train$label, 
                 k = 1, 
                 l = 0, 
                 prob = FALSE, 
                 use.all = TRUE) # this returns the assigned classes from test cases

pca_knn_conf_matrix <- table(pca_knn, reduced_test$label)
...
```{r knn threshold selection}
# KNN Model Predictions
knn_validation_preds <- predict(knn, data = task1_validation[2:ncol(task1_validation)], type = "response")$predictions

# Calculate PR Curves Using Validation Labels (y_val)


# PR Curve for Lasso    Precision-Recall
knn_validation_pr_curve <- pr.curve(scores.class0 = knn_validation_preds, weights.class0 = task1_validation$label, curve = TRUE)

cat("PR AUC (Random Forest, Validation Set):", knn_validation_pr_curve$auc.integral, "\n") # output


# Create Data Frames for Precision-Recall Curves
knn_curve_df <- data.frame(Model = "Random Forest", 
                          Recall = knn_validation_pr_curve$curve[, 1], 
                          Precision = knn_validation_pr_curve$curve[, 2], 
                          Threshold = knn_validation_pr_curve$curve[, 3])

```

```{r optimal thresholds}
# Combining data frames for elastic net and lasso
...
Classifiers to tweak: - KNN - GBDT - Random Forest
...
for (k in 2:10){
  fit <- knn3(label ~., data = task1_train, k=k)
  
  predicted <- predict(fit, newdata = task1_test, type = "prob")
  
  conf_matrix <- table(predicted[,2], task1_test$label)
...
```{r some workshop code for tuning}
