...
ggplot(gbdt_metrics, aes(x=index, F1)) + 
  geom_line() +
  geom_point() +
...
tuned_gbdt <- gbm_model <- gbm(label ~ ., data = task1_train, 
                     distribution = "bernoulli", # bernoulli or multinomial ???
                     n.trees = 1000, 
                     interaction.depth = 4, 
                     shrinkage = .025)
...
pca_svm_validation_preds <- predict(pca_svmfit, newdata = reduced_validation, probabilities = TRUE)
pca_svm_validation_conf_matrix <- table(pca_svm_validation_preds, reduced_validation$label)
pca_svm_metrics <- eval_metrics(str = "Cost = 10 (Base + PCA)", conf = pca_svm_validation_conf_matrix)
...
rbind(eval_metrics("Base SVM", svm_conf_matrix),
      eval_metrics("PCA SVM", pca_svm_conf_matrix))
```
