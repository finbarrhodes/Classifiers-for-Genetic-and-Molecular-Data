...
ggplot(data, aes(x = data[, 1])) +
  geom_bar() +
  labs(x = "Amount of Binding compunds", y = "Count") +
  theme_minimal()
dev.off()
```
...
To compare our models we measure their performance on unseen data. So we split the dataset in training validation and test data (70,15,15)
```{r}
...
validation_data <- unique_data[(end_training + 1):end_validation, ]  # 15% of the data
testing_data <- unique_data[(end_validation + 1):n, ]  # Remaining 15% of the data
...
calculate_balanced_accuracy_1 <- function(predictions, y_actual, thresholds, model_name) {
  ...
  for (threshold in thresholds) {
    ...
    # Calculate balanced accuracy and append results
...
# Predict on the test to evaluate performance 
# Lasso Model Predictions
...
The first model will use Random Forest with h2o library.
...
calculate_balanced_accuracy_1 <- function(predictions, y_actual, thresholds, model_name) {
  ...
  for (threshold in thresholds) {
    ...
    # Calculate balanced accuracy adn append
...
```{r}
#Train the model for 90% importance
...
```{r}
set.seed(12)
# Train the model with 95% importance
...
train_h2o[, "label"] <- as.factor(train_h2o[, "label"])  
val_h2o[, "label"] <- as.factor(val_h2o[, "label"])      
...
The selected model has a cumulative importance of 85% over the first set of features 

```{r}
gbm_model_selected85 <- h2o.gbm(
  x = selected_features_gbm_85,         
  y = y,                         
  training_frame = train_h2o,    
  validation_frame = val_h2o,    
  ntrees = 200,
...
cat("The Best Balanced Accuracy of Gradient Boosting Method on the test set is:", balanced_accuracy_gbm118, "\n")

```


The next utilized method is XGBoost 
...
# Custom balanced accuracy function was defined but later not utilized based on results on validation data
...
final_params_xg <- list(
  booster = "gbtree",
  objective = "binary:logistic",
  eval_metric = "logloss", #logloss is utilized over balanced_accuracy
  eta = best_model_xg$eta,                 
  max_depth = best_model_xg$max_depth,     
  subsample = best_model_xg$subsample,     
  colsample_bytree = best_model_xg$colsample_bytree, 
...
# Make predictions on the test data with default threshold
...
print("Confusion Matrix for XGB on train data:")
...
xgb.plot.importance(
  ...
  main = "Top 10 Most Important Features for XGB",
...
# Threshold optimization with validation dataset
...
for (threshold_xg in threshold_values_xg_valid) {
  ...
  # Calculate balanced accuracy and append it
...
cat("Best BA achieved on validation data is Balanced Accuracy_xg:", max_balanced_accuracy_xg$BalancedAccuracy_xg, "\n")
...
The next chapter works decreasing the amount of features selected until 118.
...
# Train a model with less features
...
for (threshold_xg in threshold_values_xg_valid) {
  ...
  # Calculate balanced accuracy
  balan_acc_xg_valid_118 <- 0.5 * (tp_xg_118 / (tp_xg_118 + fn_xg_118 + 1e-6) + tn_xg_118 / (tn_xg_118 + fp_xg_118 + 1e-6)) 
...
cat("Best Balanced Accuraacy for 136 features is:", balan_acc_xg_test_118)
...
cat("Best Balanced Accuraacy for 136 features is:", balan_acc_xg_test_136)

```

--- SUPPORT VECTOR MACHINE ---

The method utilized is Support Vector Classifier
...
features_test<- testing_data[,-1]
sparse_data_test <- as(features_test, "sparseMatrix")
# Start CV over hyperparameters for SVC
...
# Train model with best hyperparametrs
set.seed(12)
...
# Loop over the amount of features to pick the best Balanced Accuracy
k_fea <- 2
k_values <- seq(40, 90, by = 2)  
max_balanced_acc <- -Inf             
best_k_fea <- NULL                   
...
for (k_fea in k_values) {
  ...
  # Train the model for k features
  ...
  # Update the maximum balanced accuracy and best k_fea
...
cat("The amount of features is: ", best_k_fea, ", achieveing Balanced Accuracy:", max_balanced_acc, "\n")
...
# Balanced Accuracy for the best model
balan_acc_svm_valid <- 0.5 * (tp_svm_v / (tp_svm_v + fn_svm_v) + tn_svm_v / (tn_svm_v + fp_svm_v))
...
#With Validation data, lets determine the optimal threshold:
library(ggplot2)
...
for (threshold_svm in threshold_values_svm) {
  # Convert probabilities to binary predictions vs threshold
...
ggplot(threshold_results_svm, aes(x = Threshold_svm, y = BalancedAccuracy_svm)) +
  geom_line(color = "black", size = 1) +
  labs(
    title = "Balanced Accuracy vs Threshold with Maximum Balanced Accuracy for SVC",
    x = "Threshold",
    y = "Balanced Accuracy"
  ) +
  theme_minimal()
...
predictions_svm_test_list <- predict(final_model_svm_red, sparse_data_test, proba = T) 
...
cat("Best Balanced Accuracy for SVC: ",balan_acc_svm_test)
